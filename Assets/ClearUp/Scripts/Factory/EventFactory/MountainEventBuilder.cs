...
public class MountainEventBuilder : BaseEventBuilder, IEventBuilder
{
    public MountainEventBuilder(string buildtag, string cubetag,Vector3 vector3) : base(buildtag, cubetag, vector3) { }

    public override void RandomBuildingEvent()
    {
        AdventureType eventType = RandomEvent();
        //Debug.Log(eventType + "山");
        if (eventType == AdventureType.HaveSomeEnemy)
        {
            if (_CubeTag == "BlueCube")
            {
                AdventureEventManager.GetAdventureEventManager.TriggerEnemyCreature(EnemyType.Private, _Pos);
            }
            else
            {
                AdventureEventManager.GetAdventureEventManager.TriggerEnemyCreature(EnemyType.Cateran, _Pos);
            }
        }
    ...
    {
        int[] proportion = { 150,300,0,400,550,700,800,0,0,0,0,900,950,1000};
        int i = TypeRandom.ControlRandomProportion(proportion);
